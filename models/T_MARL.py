...
class SelfAttention(nn.Module):
    ...
    def forward(self, query_states, key_states, value_states):
        ...
        if self.rpe_q is not None:
            attention_scores = attention_scores + self.rpe_q(query_layer)

        # image relative position on queries
        if self.rpe_k is not None:
            attention_scores = attention_scores + self.rpe_k(key_layer).transpose(2, 3)
        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores)
        ...
        if self.rpe_v is not None:
            context_layer = context_layer + self.rpe_v(attention_probs)
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
...
class ChangeDetectorDoubleAttDyn(nn.Module):
    ...
    def __init__(self, cfg, temp=0.07):
        ...
        self.gate_dim = cfg.model.transformer_encoder.att_dim
        ...
        self.context1 = nn.Linear(self.gate_dim, self.gate_dim, bias=False)
        self.context2 = nn.Linear(self.gate_dim, self.gate_dim)

        self.gate1 = nn.Linear(self.gate_dim, self.gate_dim, bias=False)
        self.gate2 = nn.Linear(self.gate_dim, self.gate_dim)
        ...
        self.common_proj = nn.Linear(self.att_dim, 256)
    ...
    def forward(self, input_1, input_2):
        ...
        input_1 = self.ssre(input_1, input_1, input_1)
        input_2 = self.ssre(input_2, input_2, input_2)

        bef_cls_token = self.bef_cls_token.repeat(batch_size, 1, 1)
        aft_cls_token = self.aft_cls_token.repeat(batch_size, 1, 1)

        input_1 = torch.cat([bef_cls_token, input_1], 1)
        input_2 = torch.cat([aft_cls_token, input_2], 1)

        input_1 = self.encoder(input_1, input_2, input_2)
        input_2 = self.encoder(input_2, input_1, input_1)

        input_1_cls = input_1[:, 0, :]
        input_2_cls = input_2[:, 0, :]
        ...
        input_bef_context = torch.tanh(self.context1(input_diff) + self.context2(input_1))
        input_bef_context = self.dropout(input_bef_context)
        input_bef_gate = torch.sigmoid(self.gate1(input_diff) + self.gate2(input_1))
        input_bef_gate = self.dropout(input_bef_gate)
        input_befs = input_bef_gate * input_bef_context

        input_aft_context = torch.tanh(self.context1(input_diff) + self.context2(input_2))
        input_aft_context = self.dropout(input_aft_context)
        input_aft_gate = torch.sigmoid(self.gate1(input_diff) + self.gate2(input_2))
        input_aft_gate = self.dropout(input_aft_gate)
        input_afts = input_aft_gate * input_aft_context

        input_befs = input_befs.permute(0, 2, 1).view(batch_size, self.gate_dim, H, W)
        input_afts = input_afts.permute(0, 2, 1).view(batch_size, self.gate_dim, H, W)
        input_1 = input_1.permute(0, 2, 1).view(batch_size, self.gate_dim, H, W)
        input_2 = input_2.permute(0, 2, 1).view(batch_size, self.gate_dim, H, W)
        input_diff = input_diff.permute(0, 2, 1).view(batch_size, self.gate_dim, H, W)

        input_before = torch.cat([input_1, input_diff, input_befs], 1)
        input_after = torch.cat([input_2, input_diff, input_afts], 1)
        ...
        return output, att_weight_before, att_weight_after, loss_con
