...
class DecoderLayer(nn.Module):
    def __init__(self, cfg):
        ...
        self.dec_enc_attention = SelfAttention(cfg)
        ...
        self.gate2x = nn.Linear(self.hidden_size * 3,
                                self.hidden_size)

        self.norm3 = nn.LayerNorm(self.hidden_size, eps=1e-6)
    ...
    def forward(self, dec_hidden_states, dec_mask, enc_outputs, diagonal_mask=True):
        ...
        vpos = self.pos1(attention_output)
        vpos = self.dropout(self.act(vpos))
        ...
        # dec_enc_attention_output = self.norm2(attention_output + dec_enc_attention_output)

        gate_input = torch.cat([attention_output, ppos, dec_enc_attention_output], -1)

        gate = torch.sigmoid(self.gate2x(self.dropout(self.act(self.gate1x(gate_input)))))
        # dec_enc_attention_output = self.norm2(attention_output + dec_enc_attention_output)

        # dec_enc_attention_output = self.norm2(attention_output + dec_enc_attention_output)  # (N, Lt, D)
        dec_enc_attention_output = self.norm3(gate * dec_enc_attention_output + dec_enc_attention_output)
        dec_enc_attention_output = self.norm2(attention_output + dec_enc_attention_output)  # (N, Lt, D)
        ...
        return dec_enc_attention_output, dpos  # (N, Lt, D)
...
class DynamicSpeaker(nn.Module):
    ...
    def _forward(self,
                 encoder_output, seq, mask, labels_with_ignore=None, pos_labels_with_ignore=None):


        decoder_outputs, decoder_outputs_pos = self.core(
        ...
        if labels_with_ignore is not None:
            ...
            pos_loss = self.loss_func(decoder_outputs_pos.view(-1, 16),
                                          pos_labels_with_ignore.view(-1))
        return caption_loss, prediction_scores, pos_loss
    ...
    def sample(
        ...
        for dec_idx in range(max_cap_len):
            ...
            decoder_outputs, _ = self.core(text_input_ids, text_masks, encoder_outputs, diagonal_mask=True)
